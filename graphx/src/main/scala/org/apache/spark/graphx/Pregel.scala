...
object Pregel extends Logging {
    ...
    : Graph[VD, ED] =
  {
    ...
    while (activeMessages > 0 && i < maxIterations) {
      ...
      messages = g.mapReduceTriplets(sendMsg, mergeMsg, useParallel, Some((newVerts, activeDirection))).cache()
      // The call to count() materializes `messages`, `newVerts`, and the vertices of `g`. This
      // hides oldMessages (depended on by newVerts), newVerts (depended on by messages), and the
      // vertices of prevG (depended on by newVerts, oldMessages, and the vertices of g).
      activeMessages = messages.count()

      logInfo("Pregel finished iteration " + i + " number of active messages: " + activeMessages)
      ...
      println("Iteration " + i + " took " + (t1 - t0)/1000000 + " ms")
